...
function parseSseMessages(text: string): Record<string, unknown>[] {
  ...
  for (const line of text.split("\n")) {
    if (line.startsWith("data: ")) {
      try {
        messages.push(
          JSON.parse(line.slice(6).trim()) as Record<string, unknown>,
        );
      } catch {
        /* skip */
      }
...
async function serverSearch(
  ...
  if (!sid) {
    console.error(
      "No session ID returned — server may be in stateless mode without session support",
    );
  }
  ...
  if (sourceName) {
    ...
    if (tool) {
      toolCalls.push({
        name: tool.name,
        params: { query, limit: limit ?? tool.default_limit },
      });
    } else {
...
function formatResults(results: ChunkResult[]): void {
  for (let i = 0; i < results.length; i++) {
    ...
    console.log(
      `SNIPPET ${i + 1} [${similarity}% match] (source: ${r.source_name})`,
    );
    if (r.title) {
...
console.log(
  `Source: ${type ?? "all"} | Limit: ${limit ?? "default"} | Mode: ${server ? `server (${url || "localhost:3001"})` : "direct"}`,
);
console.log("");
