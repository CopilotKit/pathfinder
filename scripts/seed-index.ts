...
async function main(): Promise<void> {
  ...
  if (sourceName) {
    ...
    if (sources.length === 0) {
      ...
      console.error(
        `Error: source "${sourceName}" not found in config. Available sources: ${available}`,
      );
      process.exit(1);
    }
  }
  ...
  console.log(
    `Mode: ${sourceName ? `source=${sourceName}` : `all (${sources.map((s) => s.name).join(", ")})`}`,
  );
  console.log("");
  ...
  for (const sourceConfig of sources) {
    ...
    console.log(
      `--- Indexing source: ${sourceConfig.name} (${sourceConfig.type}) ---`,
    );
    ...
    console.log(
      `Source "${sourceConfig.name}" indexed ${result.items.length} items in ${elapsed}s\n`,
    );
  }
