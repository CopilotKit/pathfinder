...
function extractText($: CheerioAPI, el: Cheerio<AnyNode>): string {
  ...
  el.contents().each((_, node) => {
    ...
    if (tag === "pre") {
      ...
      ["p", "div", "blockquote", "dd", "section", "figcaption"].includes(tag)
    ) {
...
function splitOnHeadings(
  ...
  function walk(el: Cheerio<AnyNode>) {
    el.contents().each((_, node) => {
      ...
      if (
        ["p", "blockquote", "dd", "figcaption", "dl", "figure"].includes(tag)
      ) {
...
function splitLargeText(text: string, targetChars: number): string[] {
  ...
  if (paragraphs.length > 1) {
    return mergeSmallParts(paragraphs, targetChars).flatMap((p) =>
      splitLargeText(p, targetChars),
    );
  }
...
function mergeAndOverlap(
  ...
  targetChars: number,
  overlapChars: number,
): { content: string; sectionIndex: number }[] {
