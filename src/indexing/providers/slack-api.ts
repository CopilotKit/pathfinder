...
export class SlackApiClient {
  ...
  async fetchChannelHistory(
    ...
    do {
      ...
      if (pages >= MAX_PAGES) {
        console.warn(
          `${this.logPrefix} fetchChannelHistory hit max pages (${MAX_PAGES}) for channel ${channelId}`,
        );
        break;
      }
    } while (cursor);
  ...
  async fetchThreadReplies(
    ...
    do {
      ...
      if (pages >= MAX_PAGES) {
        console.warn(
          `${this.logPrefix} fetchThreadReplies hit max pages (${MAX_PAGES}) for thread ${threadTs}`,
        );
        break;
      }
    } while (cursor);
