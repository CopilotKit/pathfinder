...
export class NotionApiClient {
  ...
  constructor(token: string, options?: NotionApiClientOptions) {
    ...
    this.minRequestInterval =
      options?.minRequestInterval ?? DEFAULT_MIN_REQUEST_INTERVAL;
  ...
  async searchPages(editedAfter?: string): Promise<NotionPageMeta[]> {
    ...
    do {
      ...
      for (const result of response.results) {
        ...
        if (cutoff && new Date(page.last_edited_time).getTime() <= cutoff)
          continue;
      ...
      if (pageCount >= MAX_PAGES) {
        console.warn(
          `${this.logPrefix} searchPages hit max pages (${MAX_PAGES})`,
        );
        break;
      }
    } while (startCursor);
  ...
  async queryDatabase(
    ...
    do {
      ...
      if (pageCount >= MAX_PAGES) {
        console.warn(
          `${this.logPrefix} queryDatabase hit max pages (${MAX_PAGES})`,
        );
        break;
      }
    } while (startCursor);
  ...
  private async fetchBlocks(
    ...
    for (const blk of blocks) {
      ...
      if (blk.has_children) {
        ...
        const childLines = await this.fetchBlocks(
          blk.id,
          depth + 1,
          childIndent,
        );
  ...
  private async paginateBlockChildren(blockId: string): Promise<any[]> {
    ...
    do {
      ...
      if (pageCount >= MAX_PAGES) {
        console.warn(
          `${this.logPrefix} paginateBlockChildren hit max pages (${MAX_PAGES})`,
        );
        break;
      }
    } while (startCursor);
  ...
  private convertBlock(blk: any, indent: string): string | null {
    ...
    switch (type) {
      ...
      case "bookmark": {
        const bmCaption = data.caption
          ? this.renderRichText(data.caption)
          : data.url;
  ...
  private serializeProperty(prop: any): string | null {
    switch (prop.type) {
      ...
      case "multi_select":
        return (
          (prop.multi_select ?? []).map((s: any) => s.name).join(", ") || null
        );
      case "date": {
