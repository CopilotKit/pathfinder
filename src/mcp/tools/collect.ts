 ...
 */
export function yamlSchemaToZod(schema: CollectToolConfig['schema']): Record<string, z.ZodTypeAny> {
    const shape: Record<string, z.ZodTypeAny> = {};

    for (const [fieldName, field] of Object.entries(schema) as [string, FieldDef][]) {
    ...
    server.tool(
        ...
        async (input) => {
            try {
                ...
                console.error(`[${toolConfig.name}] Error inserting collected data: ${detail}`);
                return {
