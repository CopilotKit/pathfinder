...
export function generateSearchTipsMd(searchToolNames: string[]): string {
    ...
    if (searchToolNames.length > 0) {
        ...
        for (const name of searchToolNames) {
            lines.push(`- **${name}** — semantic vector search (finds related content even without exact keyword match)`);
        }
        lines.push(``);
        lines.push(`**Tip:** Use grep for exact matches, search tools for conceptual queries.`);
    }
    lines.push(`## Workflow`);
    lines.push('1. Start with `find` or `ls` to understand the directory structure');
