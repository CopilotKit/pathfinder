...
export async function crawlSite(
  ...
  const robotsResult = await rateLimitedFetch(
    `${baseUrl}/robots.txt`,
    rateLimit,
    maxRetries,
    lastFetchTime,
  ...
  const sitemapResult = await rateLimitedFetch(
    `${baseUrl}/sitemap.xml`,
    rateLimit,
    maxRetries,
    lastFetchTime,
  ...
  if (sitemapResult.ok) {
    ...
    if (indexUrls.length > 0) {
      ...
      for (const sitemapUrl of indexUrls) {
        const childResult = await rateLimitedFetch(
          sitemapUrl,
          rateLimit,
          maxRetries,
          lastFetchTime,
  ...
  const sitemapIndexResult = await rateLimitedFetch(
  ...
  if (robotsResult.ok) {
    ...
    if (sitemapUrls.length > 0) {
      ...
      for (const sitemapUrl of sitemapUrls) {
        const smResult = await rateLimitedFetch(
          sitemapUrl,
          rateLimit,
          maxRetries,
          lastFetchTime,
