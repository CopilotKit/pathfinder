...
function makeConfig(
  overrides: Partial<SourceConfig & { type: "notion" }> = {},
): SourceConfig {
  return {
    type: "notion",
    name: "test-notion",
...
describe("NotionDataProvider", () => {
  ...
  async function createProvider(config?: SourceConfig, token?: string) {
    const { NotionDataProvider } =
      await import("../indexing/providers/notion.js");
    return new NotionDataProvider(config ?? makeConfig(), {
      cloneDir: "/tmp",
      notionToken: token ?? "test-token",
    });
  }
  ...
  it("throws if notionToken is missing", async () => {
    const { NotionDataProvider } =
      await import("../indexing/providers/notion.js");
    expect(
      () => new NotionDataProvider(makeConfig(), { cloneDir: "/tmp" }),
    ).toThrow("notionToken");
  });
  ...
  it("passes max_depth from config to API client", async () => {
    ...
    expect(mockConstructorArgs).toHaveBeenCalledWith("tok-123", {
      maxDepth: 15,
    });
  });
  ...
  it("fullAcquire with include_properties=false (no header)", async () => {
    ...
    const provider = await createProvider(
      makeConfig({ databases: ["db1"], include_properties: false }),
    );
    const result = await provider.fullAcquire();
  ...
  it("fullAcquire deduplicates pages in both root_pages and database", async () => {
    ...
    const provider = await createProvider(
      makeConfig({ root_pages: ["dup1"], databases: ["db1"] }),
    );
    const result = await provider.fullAcquire();
  ...
  it("incrementalAcquire detects deleted pages via removedIds", async () => {
    ...
    mockSearchPages.mockResolvedValueOnce([
      makePage("p2", "Two", "2025-06-01T00:00:00Z"),
    ]);
  ...
  it("fullAcquire continues when root page fetch fails in discoverAllPages", async () => {
    mockGetPageMeta
      ...
      .mockResolvedValueOnce(
        makePage("rp2", "Good Page", "2025-01-01T00:00:00Z"),
      );
    mockGetPageContent.mockResolvedValue("Content");

    const provider = await createProvider(
      makeConfig({ root_pages: ["bad-id", "rp2"] }),
    );
    const result = await provider.fullAcquire();
  ...
  it("fullAcquire continues when database query fails in discoverAllPages", async () => {
    mockQueryDatabase
      ...
      .mockResolvedValueOnce([
        makePage("dp1", "DB Page", "2025-01-01T00:00:00Z"),
      ]);
    mockGetPageContent.mockResolvedValue("Content");

    const provider = await createProvider(
      makeConfig({ databases: ["bad-db", "good-db"] }),
    );
    const result = await provider.fullAcquire();
  ...
  it("incrementalAcquire with databases passes editedAfter to queryDatabase", async () => {
    ...
    expect(mockQueryDatabase).toHaveBeenNthCalledWith(
      2,
      "db1",
