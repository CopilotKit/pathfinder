...
describe("verifySlackSignature", () => {
  ...
  it("rejects invalid signature", () => {
    const body = Buffer.from('{"test": true}');
    const timestamp = String(Math.floor(Date.now() / 1000));
    expect(verifySlackSignature(body, timestamp, "v0=invalid", secret)).toBe(
      false,
    );
  });
  ...
  it("rejects missing signature", () => {
    const body = Buffer.from('{"test": true}');
    const timestamp = String(Math.floor(Date.now() / 1000));
    expect(verifySlackSignature(body, timestamp, undefined, secret)).toBe(
      false,
    );
  });
  ...
  it("rejects expired timestamp (>5 minutes old)", () => {
    ...
    expect(verifySlackSignature(body, oldTimestamp, signature, secret)).toBe(
      false,
    );
  });
});
