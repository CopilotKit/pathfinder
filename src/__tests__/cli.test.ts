...
describe("pathfinder CLI", () => {
  ...
  describe("init", () => {
    it("creates pathfinder.yaml and .env in a temp directory", () => {
      ...
      const yamlContent = fs.readFileSync(
        path.join(cwd, "pathfinder.yaml"),
        "utf-8",
      );
      const templateContent = fs.readFileSync(
        path.join(PROJECT_ROOT, "pathfinder.example.yaml"),
        "utf-8",
      );
    ...
    it("does NOT overwrite existing pathfinder.yaml", () => {
      ...
      expect(fs.readFileSync(path.join(cwd, "pathfinder.yaml"), "utf-8")).toBe(
        existingContent,
      );
    ...
    it("does NOT overwrite existing .env", () => {
      ...
      expect(fs.readFileSync(path.join(cwd, ".env"), "utf-8")).toBe(
        existingEnv,
      );
      expect(output).toContain("already exists");
    });
  ...
  describe("serve", () => {
    it("--help prints help without starting a server", () => {
      const output = execSync(`node ${CLI} serve --help`, {
        encoding: "utf-8",
      });
