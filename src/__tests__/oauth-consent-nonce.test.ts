...
function reencode(decoded: Record<string, unknown>, originalToken: string): string {
    const mac = originalToken.split(".")[1]!;
    const b64 = Buffer.from(JSON.stringify(decoded), "utf8").toString("base64url");
    return `${b64}.${mac}`;
}
...
describe("consent nonce", () => {
    ...
    it("code_challenge_method is part of the MAC", () => {
        const tok = signConsentNonce(payload({ code_challenge_method: "S256" }), [k1]);
        const decoded = decodePayload(tok);
    ...
    it("resource is part of the MAC (aud-swap prevention)", () => {
        const tok = signConsentNonce(payload({ resource: "https://x.example/r1" }), [k1]);
        const decoded = decodePayload(tok);
    ...
    it("redirect_uri is part of the MAC", () => {
        const tok = signConsentNonce(payload({ redirect_uri: "https://a.example/cb" }), [k1]);
        const decoded = decodePayload(tok);
    ...
    it("length-prefix prevents field-boundary ambiguity", () => {
        ...
        expect(__testOnly_canonicalBytes(a)).not.toEqual(__testOnly_canonicalBytes(b));
