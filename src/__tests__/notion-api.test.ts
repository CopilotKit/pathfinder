...
} from "../indexing/providers/notion-api.js";
...
function notionPage(
  ...
  title: string,
  lastEdited: string,
  parentType: "workspace" | "page_id" | "database_id" = "workspace",
  parentId?: string,
): any {
  const parent: any =
...
describe("NotionApiClient", () => {
  ...
  describe("block-to-markdown", () => {
    it("converts paragraph to plain text", async () => {
      mockBlocksChildrenList.mockResolvedValueOnce(
        blocksResponse([
          block("paragraph", {
            paragraph: { rich_text: richText("Hello world") },
          }),
        ]),
      );
    ...
    it("converts heading_2 to ## text", async () => {
      mockBlocksChildrenList.mockResolvedValueOnce(
        blocksResponse([
          block("heading_2", {
            heading_2: { rich_text: richText("Subtitle") },
          }),
        ]),
      );
      const md = await client.getPageContent("page-1");
      ...
      mockBlocksChildrenList.mockResolvedValueOnce(
        blocksResponse([
          block("bulleted_list_item", {
            bulleted_list_item: { rich_text: richText("Bullet point") },
          }),
        ]),
      );
      const md = await client.getPageContent("page-1");
      ...
      mockBlocksChildrenList.mockResolvedValueOnce(
        blocksResponse([
          block("numbered_list_item", {
            numbered_list_item: { rich_text: richText("First item") },
          }),
        ]),
      );
      const md = await client.getPageContent("page-1");
    ...
    it("converts to_do (checked) to - [x] text", async () => {
      mockBlocksChildrenList.mockResolvedValueOnce(
        blocksResponse([
          block("to_do", {
            to_do: { rich_text: richText("Done task"), checked: true },
          }),
        ]),
      );
      const md = await client.getPageContent("page-1");
    ...
    it("converts to_do (unchecked) to - [ ] text", async () => {
      mockBlocksChildrenList.mockResolvedValueOnce(
        blocksResponse([
          block("to_do", {
            to_do: { rich_text: richText("Open task"), checked: false },
          }),
        ]),
      );
      const md = await client.getPageContent("page-1");
    ...
    it("converts callout to > **icon** text", async () => {
      mockBlocksChildrenList.mockResolvedValueOnce(
        blocksResponse([
          block("callout", {
            callout: {
              rich_text: richText("Important note"),
    ...
    it("converts callout with no icon", async () => {
      mockBlocksChildrenList.mockResolvedValueOnce(
        blocksResponse([
          block("callout", {
            callout: { rich_text: richText("No icon callout"), icon: null },
          }),
        ]),
      );
      const md = await client.getPageContent("page-1");
    ...
    it("converts divider to ---", async () => {
      mockBlocksChildrenList.mockResolvedValueOnce(
        blocksResponse([block("divider", { divider: {} })]),
      );
      const md = await client.getPageContent("page-1");
      expect(md).toContain("---");
    });
    ...
    it("converts image with file URL", async () => {
      mockBlocksChildrenList.mockResolvedValueOnce(
        blocksResponse([
          block("image", {
            image: {
              type: "file",
              file: { url: "https://s3.aws.com/notion/pic.png" },
              caption: richText("Uploaded pic"),
            },
          }),
        ]),
      );
      const md = await client.getPageContent("page-1");
      expect(md).toContain(
        "![Uploaded pic](https://s3.aws.com/notion/pic.png)",
      );
    });
      ...
      mockBlocksChildrenList
        ...
        .mockResolvedValueOnce(
          blocksResponse([
            block("table_row", {
              table_row: {
                cells: [richText("Header A"), richText("Header B")],
              },
            }),
            block("table_row", {
              table_row: { cells: [richText("Cell 1"), richText("Cell 2")] },
            }),
          ]),
        );
      const md = await client.getPageContent("page-1");
    ...
    it("converts table without column header (no separator row)", async () => {
      ...
      mockBlocksChildrenList
        ...
        .mockResolvedValueOnce(
          blocksResponse([
            block("table_row", {
              table_row: { cells: [richText("A1"), richText("B1")] },
            }),
            block("table_row", {
              table_row: { cells: [richText("A2"), richText("B2")] },
            }),
          ]),
        );
      const md = await client.getPageContent("page-1");
  ...
  describe("recursion", () => {
    it("recurses into child blocks (toggle with children)", async () => {
      ...
      mockBlocksChildrenList
        ...
        .mockResolvedValueOnce(
          blocksResponse([
            block("paragraph", {
              paragraph: { rich_text: richText("Inside toggle") },
            }),
          ]),
        );
    ...
    it("respects max recursion depth", async () => {
      ...
      mockBlocksChildrenList.mockResolvedValueOnce(
        blocksResponse([
          block("paragraph", {
            paragraph: { rich_text: richText("Too deep") },
          }),
        ]),
      );
    ...
    it("respects custom maxDepth from constructor options", async () => {
      const shallowClient = new NotionApiClient("test-token", {
      ...
      mockBlocksChildrenList.mockResolvedValueOnce(
        blocksResponse([
          block("paragraph", {
            paragraph: { rich_text: richText("Too deep for shallow") },
          }),
        ]),
      );
  ...
  describe("queryDatabase", () => {
    ...
    it("does not return properties for non-database pages", async () => {
      mockSearch.mockResolvedValueOnce({
        results: [
          notionPage("p1", "Regular Page", "2024-06-01T00:00:00Z", "workspace"),
        ],
        has_more: false,
        next_cursor: null,
      });
  ...
  describe("getDatabaseMeta", () => {
    it("returns database metadata", async () => {
      ...
      expect(meta.propertyNames).toEqual(
        expect.arrayContaining(["Name", "Status", "Priority"]),
      );
    });
  });
  ...
  describe("serializeProperties", () => {
    ...
    it("serializes rich_text property", () => {
      const props = {
        Description: { type: "rich_text", rich_text: richText("Some text") },
      };
      const result = client.serializeProperties(props);
    ...
    it("serializes date property", () => {
      const props = {
        Due: { type: "date", date: { start: "2024-06-01", end: null } },
      };
      const result = client.serializeProperties(props);
    ...
    it("serializes date property with range", () => {
      const props = {
        Range: {
          type: "date",
          date: { start: "2024-06-01", end: "2024-06-30" },
        },
      };
      const result = client.serializeProperties(props);
    ...
    it("serializes created_time property", () => {
      const props = {
        Created: { type: "created_time", created_time: "2024-06-01T00:00:00Z" },
      };
      const result = client.serializeProperties(props);
  ...
  describe("MAX_PAGES safety bound", () => {
    it("searchPages stops paginating at MAX_PAGES (100)", async () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringContaining("searchPages hit max pages"),
      );
    ...
    it("queryDatabase stops paginating at MAX_PAGES (100)", async () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringContaining("queryDatabase hit max pages"),
      );
    ...
    it("paginateBlockChildren stops at MAX_PAGES (100)", async () => {
      ...
      const freshClient = new NotionApiClient("test-token", {
        minRequestInterval: 0,
      });
      mockBlocksChildrenList.mockReset();
      ...
      for (let i = 0; i < 100; i++) {
        mockBlocksChildrenList.mockResolvedValueOnce(
          blocksResponse(
            [
              block("paragraph", {
                paragraph: { rich_text: richText(`Block ${i}`) },
              }),
            ],
            true,
            `cursor-${i + 1}`,
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringContaining("paginateBlockChildren hit max pages"),
      );
        ...
        results: [
          {
            ...
            properties: {
              Description: {
                type: "rich_text",
                rich_text: richText("Just a description"),
              },
            },
          },
        ],
  ...
  describe("throttling", () => {
    it("enforces minimum delay between API calls", async () => {
      ...
      const throttledClient = new NotionApiClient("test-token", {
        minRequestInterval: 340,
      });
