...
describe("chunkRawText", () => {
  ...
  it("finds clean break point in overlap at newline", () => {
    ...
    const chunks = chunkRawText(
      content,
      "test.txt",
      mkConfig({ target_tokens: 80, overlap_tokens: 20 }),
    );
  ...
  it("respects custom target_tokens for smaller chunks", () => {
    ...
    const smallChunks = chunkRawText(
      content,
      "test.txt",
      mkConfig({ target_tokens: 50 }),
    );
    const largeChunks = chunkRawText(
      content,
      "test.txt",
  ...
  it("handles content with special characters", () => {
    const content =
      "Content with $pecial ch@rs: [brackets] {braces} (parens) & <angles>";
    const chunks = chunkRawText(content, "test.txt", mkConfig());
