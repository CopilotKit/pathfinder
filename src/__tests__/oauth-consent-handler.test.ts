...
function basePayload(over: Partial<ConsentNoncePayload> = {}): ConsentNoncePayload {
    return {
        client_id: "cid",
...
describe("consentHandler — client + policy", () => {
    it("unknown client_id (deleted between sign and consent) → 400 unauthorized_client + logs consent_stale_client", () => {
        ...
        expect(warnSpy).toHaveBeenCalledWith(
            expect.stringMatching(/^\[oauth\] consent_stale_client .*client_id=nonexistent/),
        );
        warnSpy.mockRestore();
    });
...
describe("consentHandler — invalid decision", () => {
    it("decision value neither approve nor deny → 400 + logs consent_unknown_decision", () => {
        ...
        expect(res.redirect).not.toHaveBeenCalled();
        expect(warnSpy).toHaveBeenCalledWith(
            expect.stringMatching(/^\[oauth\] consent_unknown_decision .*decision=maybe/),
        );
        warnSpy.mockRestore();
    });
