...
} from "../mcp/tools/bash-related.js";

describe("parseRelatedCommand", () => {
  it("parses valid related command", () => {
    expect(parseRelatedCommand("related /docs/quickstart.mdx")).toEqual({
      isRelated: true,
...
describe("handleRelatedCommand", () => {
  ...
  it("returns related files excluding self", async () => {
    const result = await handleRelatedCommand(
      "/docs/quickstart.mdx",
      "# Quickstart",
      mockEmbed as any,
      mockSearch,
  ...
  it("does not false-positive exclude files sharing a basename", async () => {
    const searchWithSharedBasename = vi.fn().mockResolvedValue([
      {
        ...
        content: "components",
        repo_url: null,
        file_path: "components/index.mdx",
        start_line: 1,
        end_line: 5,
        language: null,
        similarity: 0.9,
    ...
    expect(result.stdout).not.toMatch(
      /\d+\.\d+\s+\/docs\/components\/index\.mdx/,
    );
  });
