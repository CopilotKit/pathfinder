...
describe("SlackDataProvider", () => {
  ...
  beforeEach(() => {
    ...
    mockApiClient = (SlackApiClient as ReturnType<typeof vi.fn>).mock.results[0]
      .value;
  });
  ...
  describe("fullAcquire", () => {
    it("fetches threads and distills Q&A pairs", async () => {
      ...
      mockApiClient.fetchUserInfo
        ...
        .mockResolvedValueOnce({ id: "U001", displayName: "Alice" });
      mockApiClient.getChannelPermalink.mockResolvedValue(
        "https://slack.com/archives/C001/p1000",
      );
    ...
    it("stores all Q&A pairs regardless of confidence (filtering at query time)", async () => {
      ...
      mockApiClient.fetchUserInfo.mockResolvedValue({
        id: "U001",
        displayName: "User",
      });
      (distillThread as ReturnType<typeof vi.fn>).mockResolvedValue({
    ...
    it("continues processing other channels on error", async () => {
      ...
      const multiMock = (SlackApiClient as ReturnType<typeof vi.fn>).mock
        .results[
      ...
      multiMock.fetchUserInfo.mockResolvedValue({
        id: "U001",
        displayName: "User",
      });
  ...
  describe("incrementalAcquire", () => {
    it("passes oldest parameter for incremental fetch", async () => {
      ...
      expect(mockApiClient.fetchChannelHistory).toHaveBeenCalledWith(
        "C001",
        "1500.0000",
      );
    });
          ...
          new SlackDataProvider(
            {
              ...
              file_patterns: ["**/*.md"],
              chunk: {},
            } as unknown as SlackSourceConfig,
            { cloneDir: "/tmp/test" },
          ),
