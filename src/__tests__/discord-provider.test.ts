...
describe("DiscordDataProvider", () => {
  ...
  describe("constructor", () => {
    ...
    it("throws when discordBotToken is missing", () => {
      expect(
        () =>
          new DiscordDataProvider(textOnlyConfig, { cloneDir: "/tmp/test" }),
  ...
  describe("fullAcquire — text channels", () => {
    it("fetches threads and distills Q&A pairs", async () => {
      ...
      mockApiClient.fetchThreadMessages.mockResolvedValue([
        ...
        {
          id: "1002",
          author: { id: "U2", username: "bob", global_name: "Bob" },
          content: "Answer.",
      ...
      mockApiClient.fetchUser
        ...
        .mockResolvedValueOnce({ id: "U1", displayName: "Alice" });
      mockApiClient.getMessageUrl.mockReturnValue(
        "https://discord.com/channels/G001/C001/1001",
      );
  ...
  describe("fullAcquire — forum channels", () => {
    it("extracts Q&A directly from forum threads", async () => {
      ...
      mockApiClient.fetchThreadMessages.mockResolvedValue([
        {
          ...
          content: "How do I install? I need help.",
          timestamp: "2024-01-01T00:00:00Z",
        },
        {
          id: "5001",
          author: { id: "U2", username: "bob", global_name: "Bob" },
          content: "Run npm install @copilotkit/core",
          timestamp: "2024-01-01T00:01:00Z",
        },
      ]);
      mockApiClient.getMessageUrl.mockReturnValue(
        "https://discord.com/channels/G001/T1/5000",
      );
      ...
      expect(result.items[0].content).toContain(
        "Bob: Run npm install @copilotkit/core",
      );
      ...
      mockApiClient.fetchThreadMessages.mockResolvedValue([
        {
          ...
          author: { id: "U1", username: "alice", global_name: "Alice" },
          content:
            "How do I install? I've been trying for hours and nothing works.",
          timestamp: "2024-01-01T00:00:00Z",
        },
        {
          id: "5001",
    ...
    it("truncates long forum answers at MAX_ANSWER_CHARS", async () => {
      ...
      mockApiClient.getMessageUrl.mockReturnValue(
        "https://discord.com/channels/G001/T1/6000",
      );
  ...
  describe("fullAcquire — mixed channels", () => {
    it("processes both text and forum channels", async () => {
      ...
      mockApiClient.fetchUser.mockResolvedValue({
        id: "U1",
        displayName: "User",
      });
      mockApiClient.getMessageUrl.mockReturnValue(
      ...
      mockApiClient.fetchThreadMessages.mockResolvedValueOnce([
        {
          id: "9000",
          author: { id: "U1", username: "a", global_name: "UserA" },
          content: "Details here",
          timestamp: "2024-01-01T00:00:00Z",
        },
        {
          id: "9001",
          author: { id: "U2", username: "b", global_name: "UserB" },
          content: "The answer",
          timestamp: "2024-01-01T00:01:00Z",
        },
      ]);
  ...
  describe("fullAcquire — error handling", () => {
    it("throws when all channels fail", async () => {
      const provider = createProvider(textOnlyConfig);
      mockApiClient.fetchChannelMessages.mockRejectedValue(
        new Error("channel_not_found"),
      );
      await expect(provider.fullAcquire()).rejects.toThrow(
        "All 1 channel(s) failed",
      );
    });
    ...
    it("continues when one channel fails in multi-channel config", async () => {
      ...
      mockApiClient.fetchThreadMessages.mockResolvedValue([
        {
          id: "9000",
          author: { id: "U1", username: "a", global_name: "A" },
          content: "Details",
          timestamp: "2024-01-01T00:00:00Z",
        },
        {
          id: "9001",
          author: { id: "U2", username: "b", global_name: "B" },
          content: "Answer",
          timestamp: "2024-01-01T00:01:00Z",
        },
      ]);
      mockApiClient.getMessageUrl.mockReturnValue(
        "https://discord.com/channels/G001/F001/9000",
      );
  ...
  describe("incrementalAcquire", () => {
    it("passes after parameter for text channels", async () => {
      ...
      expect(mockApiClient.fetchChannelMessages).toHaveBeenCalledWith(
        "C001",
        "999",
      );
    });
    ...
    it("filters forum threads by last_message_id > lastStateToken via BigInt", async () => {
      ...
      mockApiClient.fetchThreadMessages.mockResolvedValue([
        ...
        {
          id: "1500",
          author: { id: "U3", username: "c", global_name: "C" },
          content: "Reply",
          timestamp: "2024-01-02T00:01:00Z",
        },
      ]);
      mockApiClient.getMessageUrl.mockReturnValue(
        "https://discord.com/channels/G001/T2/1400",
      );
