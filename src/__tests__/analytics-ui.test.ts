...
describe("analytics dashboard UI — low-confidence stat card", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  /**
   * Endpoints whose summary carries an explicit low-confidence shape. Pass
   * `scored: undefined` to simulate a server build that predates
   * `scored_query_count_window`.
   */
  function endpointsWithLowConfidence(opts: {
    scored?: number;
    lowConfidenceCount?: number;
    lowConfidenceRate?: number;
  }) {
    const base = canned(7, 1000).summary;
    return {
      "/api/analytics/auth-mode": () => ({ dev: true }),
      "/api/analytics/summary": () => ({
        ...base,
        low_confidence_count_window: opts.lowConfidenceCount ?? 0,
        low_confidence_rate_window: opts.lowConfidenceRate ?? 0,
        ...(opts.scored === undefined
          ? {}
          : { scored_query_count_window: opts.scored }),
      }),
      "/api/analytics/tool-counts": () => [{ tool_type: "search", count: 1 }],
      "/api/analytics/queries": () => [],
      "/api/analytics/empty-queries": () => [],
      "/api/analytics/blocked-queries": () => [],
    };
  ...
  /** The rendered stat card whose label starts with "Low Confidence". */
  function lowConfidenceCard(dom: JSDOM): HTMLElement | undefined {
    return Array.from(
      dom.window.document.querySelectorAll<HTMLElement>(".stat-card"),
    ).find((el) =>
      (el.querySelector(".label")?.textContent ?? "").startsWith(
        "Low Confidence",
      ),
    );
  }

  it("renders the low-confidence rate when the window contains cosine-scored traffic", async () => {
    const { dom } = await loadDashboard(
      endpointsWithLowConfidence({
        scored: 400,
        lowConfidenceCount: 65,
        lowConfidenceRate: 0.13,
      }),
    );

    const card = lowConfidenceCard(dom);
    expect(card).toBeDefined();
    expect(card!.querySelector(".label")!.textContent).toBe(
      "Low Confidence (Last 7 days)",
    );
    expect(card!.querySelector(".value")!.textContent).toBe("13.0%");
    // The tooltip carries the raw count so the percentage is auditable.
    expect(card!.getAttribute("title")).toContain("65");
  });

  it("renders an em-dash, NOT '0.0%', when nothing in the window carried a cosine score", async () => {
    // This is the state EVERY existing install lands in the moment score_kind
    // ships: no row declares the cosine scale yet, so low_confidence is 0 for
    // want of measurement, not for want of low-confidence queries. Rendering
    // "0.0%" there would tell the operator that nothing looks irrelevant —
    // the opposite of the truth, which is that nothing has been measured.
    const { dom } = await loadDashboard(
      endpointsWithLowConfidence({
        scored: 0,
    ...
    const card = lowConfidenceCard(dom);
    expect(card).toBeDefined();
    expect(card!.querySelector(".value")!.textContent).toBe("—");
    expect(card!.querySelector(".value")!.textContent).not.toContain("0.0%");
    expect(card!.getAttribute("title")).toMatch(/no.*cosine/i);
  });

  it("renders an em-dash when the server omits scored_query_count_window entirely", async () => {
    // An older server (or a cached page against a newer one) sends no scored
    // population. Absent evidence of measurement must degrade to the same
    // "nothing to report" glyph rather than asserting a clean 0%.
    const { dom } = await loadDashboard(
      endpointsWithLowConfidence({
        lowConfidenceCount: 0,
        lowConfidenceRate: 0,
  ...
  it("preview mode (?preview=1) ships a low-confidence figure in its canned summary", async () => {
    // The preview mockup is how the dashboard gets visually reviewed. A card
    // missing from the canned payload renders as a permanent em-dash there
    // and the surface silently stops being evaluated.
    const html = fs.readFileSync(
      path.join(process.cwd(), "docs", "analytics.html"),
      "utf8",
    );
    expect(html).toMatch(/scored_query_count_window:\s*\d+/);
    expect(html).toMatch(/low_confidence_rate_window:\s*[\d.]+/);
  });
});
