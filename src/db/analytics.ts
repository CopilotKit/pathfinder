...
export async function logQuery(
  ...
  // topCosineScore in src/mcp/tools/search.ts), and an absent score is always
  ...
  const scoreKind =
    entry.top_score == null
      ? null
      : (entry.score_kind ?? COSINE_SCORE_KIND);
  try {
    await pool.query(
...
export async function getBlockedQueries(
  ...
  // correlated subquery filters on — without the CTE, Postgres rejects the
  // correlated reference as an ungrouped column (the outer GROUP BY is on
  // `COALESCE(block_reason, '<unknown>')`, not raw `block_reason`).
  const { rows } = await pool.query(
    `WITH blocked_rows AS (
       SELECT
         COALESCE(block_reason, '<unknown>') AS block_reason_key,
         query_text,
         created_at
       FROM query_log
       WHERE blocked = true
     ...
     SELECT
        block_reason_key AS block_reason,
        count(*)::int AS hits,
        max(created_at)::text AS last_seen,
        (
          ...
          FROM (
            ...
            FROM blocked_rows AS inner_q
            WHERE inner_q.block_reason_key = blocked_rows.block_reason_key
            ORDER BY query_text
            LIMIT 5
          ) sub
        ) AS sample_queries
     FROM blocked_rows
     GROUP BY block_reason_key
     ORDER BY hits DESC`,
    [String(days)],
  );
