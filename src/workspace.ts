...
export class WorkspaceManager {
  ...
  constructor(baseDir: string, maxBytesPerSession: number = 1024 * 1024) {
    ...
    try {
      ...
      console.error(
        `[workspace] Failed to create base directory ${baseDir}: ${detail}`,
      );
      throw new Error(
        `WorkspaceManager: cannot create base directory: ${detail}`,
      );
    }
  }
  ...
  ensureSession(sessionId: string): string {
    ...
    try {
      ...
      const detail = err instanceof Error ? err.message : String(err);
      console.error(
        `[workspace] Failed to create session directory ${dir}: ${detail}`,
      );
      throw new Error(
        `WorkspaceManager: cannot create session directory: ${detail}`,
      );
    }
    if (!this.trackers.has(sessionId)) {
      this.trackers.set(
        sessionId,
        new WorkspaceTracker(this.maxBytesPerSession),
      );
    }
    return dir;
  }
  ...
  writeFile(sessionId: string, filename: string, content: string): boolean {
    ...
    if (
      !path.resolve(filePath).startsWith(sessionDir + path.sep) &&
      path.resolve(filePath) !== sessionDir
    ) {
      return false;
    }
  ...
  listFiles(sessionId: string, subdir?: string): string[] {
    ...
    if (subdir) {
      ...
      const sessionDir = path.resolve(this.baseDir, sessionId);
      if (
        !path.resolve(dir).startsWith(sessionDir + path.sep) &&
        path.resolve(dir) !== sessionDir
      ) {
        return [];
      }
    }
  ...
  cleanup(sessionId: string): void {
    ...
    try {
      ...
      const detail = err instanceof Error ? err.message : String(err);
      console.error(
        `[workspace] Failed to cleanup session ${sessionId}: ${detail}`,
      );
    }
  ...
  cleanupAll(): void {
    ...
    for (const entry of entries) {
      try {
        fs.rmSync(path.join(this.baseDir, entry), {
          recursive: true,
          force: true,
        });
      } catch (err) {
        const detail = err instanceof Error ? err.message : String(err);
        console.error(
          `[workspace] Failed to cleanup entry ${entry}: ${detail}`,
        );
      }
    }
    this.trackers.clear();
